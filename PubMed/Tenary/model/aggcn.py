...
class GCNRelationModel(nn.Module):
    ...
    def forward(self, inputs):
        words, masks, pos, deprel, head, first_pos, second_pos, third_pos = inputs  # unpack
        ...
        # pooling
        first_mask, second_mask, third_mask = first_pos.eq(0).eq(0).unsqueeze(2), second_pos.eq(0).eq(0).unsqueeze(2), third_pos.eq(0).eq(0).unsqueeze(2) # invert mask
        pool_type = self.opt['pooling']
        h_out = pool(h, pool_mask, type=pool_type)
        first_out = pool(h, first_mask, type=pool_type)
        second_out = pool(h, second_mask, type=pool_type)
        third_out = pool(h, third_mask, type=pool_type)

        outputs = torch.cat([h_out, first_out, second_out, third_out], dim=1)
